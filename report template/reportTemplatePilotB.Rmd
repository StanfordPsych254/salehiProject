...
To detect the Cohen's d effect size of 0.71 at the power of 0.8,66 participants are needed. However, it should be mentioned that this is the effect size of a sub-set of analyses and the effect size of the main mixed-model ANOVA might be below 0.71. Therefore, to assure that the replication study would not be underpowered, the replication will be conducted with 80 participants - the sample size used in the original study.
...
This data has been plotted in the following graph. This is the graph also presented in the original paper.

#### Plot of average ratings: The original paper graph
...
```{r lmer basic}
#mixed-effect model with trait type and condition only
...
I have contacted the authors asking for the exclusion policy as well as their materials for the first study. Shortly, the authors replied in regard of exclusion policy and generously provided the survey of the first study on the Qualtrics platform. In the original study, there was no participants being excluded.
